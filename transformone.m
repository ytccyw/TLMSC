...
for v=1:V
    Hnei{v} = neinei(SS{v});%  Get Nearest Neighbors
...
embedding = py.sklearn.decomposition.PCA(pyargs('n_components',int64(n_dims))).fit_transform(inputs{1});
embedding=py.numpy.ascontiguousarray(embedding);
embedding = INIT_SCALE .*double(embedding);
clear inputs
